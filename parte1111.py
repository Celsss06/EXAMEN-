...
class Registro(Frame):
    ...
    def create_wietgs(self):
        Label(self.frame1, text = 'L I C O R E R I A \t D O Ñ A \t M A D E',bg='gray99',fg='black').grid(column=0, row=0)
        ...
        Label(self.frame4, text = 'Control',fg='white', bg ='cyan').grid(columnspan=3, column=0,row=0, pady=1, padx=4)
        Button(self.frame4,command= self.agregar_datos, text='REGISTRAR', bg='DodgerBlue2').grid(column=0,row=1, pady=10, padx=4)
        Button(self.frame4,command = self.limpiar_datos, text='LIMPIAR', bg='SteelBlue2').grid(column=1,row=1, padx=10)
        Button(self.frame4,command = self.eliminar_fila, text='ELIMINAR', bg='red').grid(column=2,row=1, padx=4)
        Button(self.frame4,command = self.mostrar_todo, text='MOSTRAR DATOS DE MYSQL', bg='dark turquoise').grid(columnspan=3,column=0,row=3, pady=8)
        ...
        estilo.configure(".", foreground='cadet blue')
        estilo.configure("Treeview", foreground='black',  background='white')
        estilo.map('Treeview',background=[('selected', 'aquamarine')], foreground=[('selected','black')] )
